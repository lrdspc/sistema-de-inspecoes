...
export async function generatePDF({ relatorio, vistoria, cliente }: GeneratePDFOptions) {
  const pdf = new jsPDF({
  ...
  addText(`Data: ${format(new Date(relatorio.dataCriacao), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}`);
  yPos += 10;
  ...
  addText(`Endereço: ${cliente.logradouro}, ${cliente.numero}${cliente.complemento ? `, ${cliente.complemento}` : ''}`);
  addText(`${cliente.bairro} - ${cliente.cidade}/${cliente.estado} - CEP: ${cliente.cep}`);
  yPos += 10;

  // Dados da Vistoria
  addSubtitle('Dados da Vistoria');
  addText(`Data da Vistoria: ${format(new Date(vistoria.data), "dd/MM/yyyy 'às' HH:mm")}`);
  ...
  const parecerLabel = relatorio.parecerFinal === 'procedente' ? 'Procedente' :
                      relatorio.parecerFinal === 'improcedente' ? 'Improcedente' : 
  ...
  if (vistoria.assinaturaTecnico || vistoria.assinaturaCliente) {
    ...
    if (vistoria.assinaturaCliente) {
      pdf.addImage(vistoria.assinaturaCliente, 'PNG', margin + 100, yPos - 35, 70, 30);
      yPos += 35;
      pdf.setFontSize(10);
